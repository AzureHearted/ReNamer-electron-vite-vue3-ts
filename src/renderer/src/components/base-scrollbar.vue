<template>
  ...
  >
    ...
    <div ref="wrapDOM" class="base-scrollbar__wrap">
      <!--s 视口区内部区域 -->
      <div ref="viewDOM" class="base-scrollbar__view" @transitionend="handleUpdate()">
        <!--t 调试区域 -->
        <div v-if="false" class="debug">
          containerInfo {{ containerInfo.height }},{{ containerInfo.width }}<br />
          ...
          scrollbarLength {{ scrollbar.vertical.length }},{{ scrollbar.horizontal.length }}
        </div>
        <!--s 插槽出口 (内容) -->
        <slot></slot>
      </div>
      ...
      <transition name="back-top">
        ...
        >
          <i class="back-top__icon">
            <svg data-v-e6ff9537="" viewBox="0 0 1024 1024" width="1.2em" height="1.2em">
...
const calcVerticalBarPosition = async (y: number) => {
  ...
  const left = (scrollWidth / containerInfo.width) * scrollbar.horizontal.left
  // 更新滚动距离
  nextTick(() => {
...
const verticalScrollbarVisible = computed<boolean>(() => {
  return (
    ((scrollbar.show && scrollbar.vertical.length > 0) || scrollbar.vertical.isDragging) &&
    props.showScrollbar
  )
})
...
const horizontalScrollbarVisible = computed<boolean>(() => {
  return (
    ((scrollbar.show && scrollbar.horizontal.length > 0) || scrollbar.horizontal.isDragging) &&
    props.showScrollbar
  )
})
...
async function setScrollbarPosition() {
  ...
  const { scrollWidth, scrollHeight } = wrapDOM.value // 提取滚动容器内容区宽高
  // 更新滚动条位置
  scrollbar.vertical.y = (wrapHeight / scrollHeight) * wrapperY.value * (channelHeight / wrapHeight)
  scrollbar.horizontal.x = (wrapWidth / scrollWidth) * wrapperX.value * (channelWidth / wrapWidth)
